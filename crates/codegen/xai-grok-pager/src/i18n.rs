...
pub enum TextKey {
    ...
    // New UI strings for Modals and Views
    ModalTitleWorkflows,
    ModalTitleChangeDirectory,
    ModalTitleAgents,
    ModalTitleWelcome,
    TabContextUsage,
    TabUsageLimit,
...
pub fn tr_for(language: Language, key: TextKey) -> Cow<'static, str> {
    let text = match (language, key) {
        ...
        (Language::English, TextKey::ModalTitleWorkflows) => "Workflows",
        (Language::Russian, TextKey::ModalTitleWorkflows) => "Воркфлоу",
        (Language::English, TextKey::ModalTitleChangeDirectory) => "Change directory",
        (Language::Russian, TextKey::ModalTitleChangeDirectory) => "Сменить каталог",
        (Language::English, TextKey::ModalTitleAgents) => "Agents",
        (Language::Russian, TextKey::ModalTitleAgents) => "Агенты",
        (Language::English, TextKey::ModalTitleWelcome) => "Welcome to Grok Build",
        (Language::Russian, TextKey::ModalTitleWelcome) => "Добро пожаловать в Grok Build",

        (Language::English, TextKey::TabContextUsage) => "Context usage",
        (Language::Russian, TextKey::TabContextUsage) => "Использование контекста",
        (Language::English, TextKey::TabUsageLimit) => "Usage limit",
        (Language::Russian, TextKey::TabUsageLimit) => "Лимит использования",
        (Language::English, TextKey::TabSessionInfo) => "Session info",
        (Language::Russian, TextKey::TabSessionInfo) => "Информация о сессии",

        (Language::English, TextKey::HintTabSwitch) => "Tab switch",
        (Language::Russian, TextKey::HintTabSwitch) => "Tab смена вкладки",
        (Language::English, TextKey::HintUpDnScroll) => "↑/↓ scroll",
        (Language::Russian, TextKey::HintUpDnScroll) => "↑/↓ прокрутка",
        (Language::English, TextKey::HintCopySessionId) => "c copy session ID",
        (Language::Russian, TextKey::HintCopySessionId) => "c копировать ID",
        (Language::English, TextKey::HintCopyAll) => "y copy all",
        (Language::Russian, TextKey::HintCopyAll) => "y копировать всё",
        (Language::English, TextKey::HintEscClose) => "Esc close",
        (Language::Russian, TextKey::HintEscClose) => "Esc закрыть",

        (Language::English, TextKey::HintPhaseEnterAgent) => "↑↓ phase · enter agent",
        (Language::Russian, TextKey::HintPhaseEnterAgent) => "↑↓ фаза · enter агент",
        (Language::English, TextKey::HintTabRuns) => "←/tab runs",
        (Language::Russian, TextKey::HintTabRuns) => "←/tab запуски",
        (Language::English, TextKey::HintPause) => "p pause",
        (Language::Russian, TextKey::HintPause) => "p пауза",
        (Language::English, TextKey::HintResume) => "r resume",
        (Language::Russian, TextKey::HintResume) => "r продолжить",
        (Language::English, TextKey::HintStop) => "x stop",
        (Language::Russian, TextKey::HintStop) => "x остановить",
        (Language::English, TextKey::HintSave) => "s save",
        (Language::Russian, TextKey::HintSave) => "s сохранить",
        (Language::English, TextKey::HintUpDnSelect) => "↑↓ select",
        (Language::Russian, TextKey::HintUpDnSelect) => "↑↓ выбор",
        (Language::English, TextKey::HintEnterOpen) => "Enter open",
        (Language::Russian, TextKey::HintEnterOpen) => "Enter открыть",

        (Language::English, TextKey::HintGoDeeper) => "d go deeper",
        (Language::Russian, TextKey::HintGoDeeper) => "d подробнее",
        (Language::English, TextKey::HintEscList) => "Esc list",
        (Language::Russian, TextKey::HintEscList) => "Esc к списку",
        (Language::English, TextKey::HintUpDnNavigate) => "↑/↓ navigate",
        (Language::Russian, TextKey::HintUpDnNavigate) => "↑/↓ навигация",
        (Language::English, TextKey::HintEscDone) => "Esc done",
        (Language::Russian, TextKey::HintEscDone) => "Esc готово",

        (Language::English, TextKey::HintEscBack) => "Esc back",
        (Language::Russian, TextKey::HintEscBack) => "Esc назад",
        (Language::English, TextKey::HintCtrlClose) => "Ctrl+./X close",
        (Language::Russian, TextKey::HintCtrlClose) => "Ctrl+./X закрыть",
        (Language::English, TextKey::HintExpandShortcuts) => "e/Space/→ expand",
        (Language::Russian, TextKey::HintExpandShortcuts) => "e/Space/→ развернуть",
        (Language::English, TextKey::HintCollapseShortcuts) => "← collapse",
        (Language::Russian, TextKey::HintCollapseShortcuts) => "← свернуть",
        (Language::English, TextKey::HintEnterDetails) => "Enter details",
        (Language::Russian, TextKey::HintEnterDetails) => "Enter детали",
        (Language::English, TextKey::HintSlashSearch) => "/ search",
        (Language::Russian, TextKey::HintSlashSearch) => "/ поиск",

        (Language::English, TextKey::HintYReset) => "y reset",
        (Language::Russian, TextKey::HintYReset) => "y сбросить",
        (Language::English, TextKey::HintNCancel) => "n cancel",
        (Language::Russian, TextKey::HintNCancel) => "n отмена",
        (Language::English, TextKey::HintEscCancel) => "Esc cancel",
        (Language::Russian, TextKey::HintEscCancel) => "Esc отмена",
        (Language::English, TextKey::HintF2Cancel) => "F2 cancel",
        (Language::Russian, TextKey::HintF2Cancel) => "F2 отмена",
        (Language::English, TextKey::HintTopBtmSettings) => "g/G top/btm",
        (Language::Russian, TextKey::HintTopBtmSettings) => "g/G начало/конец",
        (Language::English, TextKey::HintSpaceToggle) => "Space toggle",
        (Language::Russian, TextKey::HintSpaceToggle) => "Space переключить",
        (Language::English, TextKey::HintDReset) => "d reset",
        (Language::Russian, TextKey::HintDReset) => "d сброс",
        (Language::English, TextKey::HintF2EscClose) => "F2/Esc close",
        (Language::Russian, TextKey::HintF2EscClose) => "F2/Esc закрыть",
        (Language::English, TextKey::HintTypeToFilter) => "type to filter",
        (Language::Russian, TextKey::HintTypeToFilter) => "введите для фильтрации",
        (Language::English, TextKey::HintBackspaceEdit) => "Backspace edit",
        (Language::Russian, TextKey::HintBackspaceEdit) => "Backspace изменить",
        (Language::English, TextKey::HintEnterCommit) => "Enter commit",
        (Language::Russian, TextKey::HintEnterCommit) => "Enter применить",
        (Language::English, TextKey::HintEscClear) => "Esc clear",
        (Language::Russian, TextKey::HintEscClear) => "Esc очистить",
        (Language::English, TextKey::HintEnterSelect) => "Enter select",
        (Language::Russian, TextKey::HintEnterSelect) => "Enter выбрать",
        (Language::English, TextKey::HintTypeToEdit) => "type to edit",
        (Language::Russian, TextKey::HintTypeToEdit) => "введите для изменения",
        (Language::English, TextKey::HintSpaceEnterToggle) => "Space/Enter toggle",
        (Language::Russian, TextKey::HintSpaceEnterToggle) => "Space/Enter переключить",
    };
...
/// Return localized description for slash command based on canonical name.
pub fn slash_command_description<'a>(canonical_name: &str, fallback: &'a str) -> std::borrow::Cow<'a, str> {
    if language() != Language::Russian {
        return std::borrow::Cow::Borrowed(fallback);
    }

    let normalized = canonical_name.trim_start_matches('/').replace('_', "-");
    let ru = match normalized.as_str() {
        "always-approve" => "Переключить автоподтверждение действий инструментов в этой сессии",
        "announcements" => "Показать анонсы и обновления продукта",
        "auto" => "Автономный режим (агент работает без запроса подтверждений)",
        "btw" => "Отправить фоновую мысль агенту без прерывания текущего шага",
        "cd" => "Сменить текущую рабочую директорию",
        "compact" => "Сжать историю диалога для уменьшения размера контекста",
        "compact-mode" => "Переключить компактный режим интерфейса (уменьшенные отступы)",
        "config-agents" => "Настроить доступных субагентов и роли",
        "context" => "Показать подробную информацию об использовании контекста и токенов",
        "copy" => "Скопировать последний ответ или весь диалог в буфер обмена",
        "dashboard" | "agents-dashboard" | "sessions" => {
            "Открыть панель агентов — полноэкранный обзор всех запущенных сессий"
        }
        "debug" => "Внутренняя диагностика и дамп состояния",
        "delete" => "Удалить текущую сессию или субагента",
        "docs" => "Открыть документацию или руководство пользователя",
        "doctor" => "Диагностика окружения, MCP-серверов и интеграций",
        "edit-prompt" => "Редактировать текущий запрос во внешнем редакторе",
        "effort" => "Установить уровень рассуждений модели (effort)",
        "exit" | "quit" => "Выйти из приложения",
        "expand" => "Развернуть или свернуть детали вызова инструментов в истории",
        "export" => "Экспортировать историю диалога в Markdown, JSON или HTML",
        "feedback" => "Отправить отзыв о текущей сессии или сообщить об ошибке",
        "find" => "Поиск по истории диалога",
        "fork" => "Создать ветку (форк) текущей сессии",
        "gboom" => "Тестовый запуск анимации или отчета об ошибках",
        "help" => "Показать доступные команды, горячие клавиши и справку",
        "history" => "Просмотр и поиск прошлых сессий диалога",
        "home" => "Прокрутить в самое начало истории диалога",
        "imagine" => "Сгенерировать изображение по текстовому описанию",
        "imagine-video" => "Сгенерировать видео по запросу или изображению",
        "import-claude" => "Импортировать историю сессий или настройки из Claude",
        "jump" => "Перейти к определенному сообщению или шагу в истории",
        "login" => "Войти в аккаунт",
        "logout" => "Выйти из аккаунта",
        "loop" => "Запустить цикл агента до выполнения условия",
        "mcps" | "mcp" => "Управление серверами и инструментами MCP (Model Context Protocol)",
        "model" => "Переключить активную модель ИИ или пресет рассуждений",
        "multiline" => "Переключить многострочный ввод (Enter — новая строка, Ctrl+Enter — отправка)",
        "new" => "Начать новую сессию диалога",
        "personas" => "Переключить персону или системные инструкции",
        "plan" => "Включить режим планирования для многошаговых задач",
        "plugin" | "plugins" => "Управление установленными плагинами и расширениями",
        "privacy" => "Настройки конфиденциальности и телеметрии",
        "queue" => "Просмотр и управление очередью сообщений и фоновых задач",
        "recap" => "Сформировать краткий итог последних действий и выполненных шагов",
        "release-notes" => "Посмотреть список изменений в этой версии",
        "remember" => "Сохранить заметку или воспоминание для будущих сессий",
        "rename" => "Переименовать текущую сессию",
        "resume" => "Возобновить приостановленную или прерванную сессию",
        "rewind" => "Откатить диалог к предыдущему сообщению",
        "screen-mode" => "Переключить полноэкранный и компактный режимы экрана",
        "scroll-debug" => "Переключить отображение отладки прокрутки",
        "session-info" => "Показать подробную информацию о текущей сессии",
        "settings" => "Открыть настройки и конфигурацию",
        "share" => "Создать ссылку или файл для экспорта сессии",
        "tasks" => "Просмотр и управление фоновыми задачами и субагентами",
        "theme" => "Сменить цветовую тему и подсветку синтаксиса",
        "timeline" => "Показать временную шкалу событий сессии",
        "timestamps" => "Переключить отображение времени сообщений",
        "toggle-mouse-reporting" | "mouse" => "Переключить захват мыши терминалом и прокрутку колесиком",
        "transcript" => "Просмотреть полный необработанный лог диалога",
        "tutorial" => "Запустить интерактивное руководство пользователя",
        "usage" => "Показать расход токенов, лимиты и баланс аккаунта",
        "view-plan" => "Просмотреть текущий план задачи и прогресс выполнения",
        "vim-mode" | "vim" => "Переключить режим управления клавишами Vim",
        "voice" => "Переключить голосовой ввод / диктовку",
        "workflows" | "workflow" => "Управление рабочими процессами и сценариями",
        _ => return std::borrow::Cow::Borrowed(fallback),
    };

    std::borrow::Cow::Borrowed(ru)
}

/// Return localized description for slash command argument suggestions.
pub fn slash_arg_description(display: &str, fallback: &str) -> String {
    if language() != Language::Russian {
        return fallback.to_string();
    }

    match display.to_ascii_lowercase().as_str() {
        "dark" => "Классическая темная тема".to_string(),
        "light" => "Светлая чистая тема".to_string(),
        "groknight" => "Глубокая ночная тема".to_string(),
        "solarized-dark" => "Темная тема Solarized".to_string(),
        "solarized-light" => "Светлая тема Solarized".to_string(),
        "dracula" => "Тема Dracula".to_string(),
        "nord" => "Арктическая тема Nord".to_string(),
        "monokai" => "Классическая тема Monokai".to_string(),
        "gruvbox" | "gruvbox-dark" => "Тема Gruvbox".to_string(),
        "github-dark" => "Тема GitHub Dark".to_string(),
        "github-light" => "Тема GitHub Light".to_string(),
        "models" => "Показать разбивку расхода токенов по моделям".to_string(),
        "limits" => "Показать текущие лимиты запросов и квоты".to_string(),
        "all" => "Запустить все доступные проверки".to_string(),
        "markdown" | "md" => "Экспорт в файл формата Markdown".to_string(),
        "json" => "Экспорт истории диалога в JSON".to_string(),
        "html" => "Экспорт в интерактивный HTML-файл".to_string(),
        "low" => "Низкий уровень рассуждений (быстрые ответы)".to_string(),
        "medium" => "Средний уровень рассуждений (баланс)".to_string(),
        "high" => "Высокий уровень рассуждений (глубокий анализ)".to_string(),
        "max" => "Максимальный уровень рассуждений".to_string(),
